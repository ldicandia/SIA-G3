...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        event = GenerationEvent(
            ...
            mean_fitness=float(population.fitness.mean()),
            worst_fitness=float(population.fitness.min()),
            diversity=float(population.genes.std(axis=0).mean()),
            active_triangles=active_count(best_genes),
        ...
        write_run_json(
            run_dir / "run.json",
            {"image": args.image, "canvas": args.canvas, "triangles": args.triangles, "population": args.population},
            seed,
