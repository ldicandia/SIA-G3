...
# Baseline seed 42 is Plan 07-01's already-trained "more data" baseline, reused
# (never retrained) from runs/more_digits/more_data_baseline{,_val}.
NEW_SEED_VARIANTS = [7, 123, 2026]
...
def main():
    ...
    for s in NEW_SEED_VARIANTS:
        run_name = f"seed_{s}"
        print(f"Training seed variant: {run_name} (seed={s})...")
        ...
        variants.append(summarize_variant(run_name, s, base_layer_sizes, train_json, val_json))

    # Baseline seed=42 - reuse Plan 07-01's already-trained runs, never retrain.
    baseline_train_runs = [
    ...
    variants.append(
        summarize_variant(
            "more_data_baseline", BASELINE_SEED, base_layer_sizes, baseline_train, baseline_val
        )
    )

    variants.sort(key=lambda v: v["seed"])

    out_path = Path("docs/ejercicio3/seed_variants.json")
    out_path.parent.mkdir(parents=True, exist_ok=True)
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump({"variants": variants}, f, indent=2)

    print(f"Seed sweep complete! Saved to {out_path}")
    for v in variants:
        print(f"  seed={v['seed']}: val_acc={v['val_accuracy']:.4f}")
